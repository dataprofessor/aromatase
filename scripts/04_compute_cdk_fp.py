"""
Compute CDK Fingerprinter (1024 bits) using PaDEL batch mode with SMILES input.

Uses PaDEL-Descriptor's Fingerprinter. SMILES (.smi) input is used instead of
SDF to ensure consistent aromaticity perception.

Note: A custom descriptors.xml is generated at runtime to enable
Fingerprinter and disable other fingerprint types.
"""
...
OUTPUT_FILE = os.path.join(_DIR, "..", "data", "fingerprints", "aromatase_cdk_fp.csv")

NBITS = 1024
FP_COLS = [f"FP{i}" for i in range(1, NBITS + 1)]
...
def make_descriptor_xml(output_path):
    """Create a custom descriptors.xml enabling only Fingerprinter."""
    import importlib
...
def main():
    ...
    # Run PaDEL batch
    print(f"\nRunning PaDEL batch CDK Fingerprinter calculation (SMILES input)...")
    start = time.time()
    ...
    print(f"  Mapped {len(fp_map)} molecules to FP")
    ...
    print(f"  Columns: {len(out_fieldnames)} (1 ID + {NBITS} FP bits)")
    print(f"  Rows with FP: {len(rows) - missing_fp}")
    print(f"  Rows without FP: {missing_fp}")
