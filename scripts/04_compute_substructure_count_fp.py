"""
Compute Substructure Count (307 counts) using PaDEL batch mode with SMILES input.

Uses PaDEL-Descriptor's SubstructureFingerprintCount. These are frequency counts (not binary).
...
OUTPUT_FILE = os.path.join(_DIR, "..", "data", "fingerprints", "aromatase_substructure_count_fp.csv")

NBITS = 307
FP_COLS = [f"SubFPC{i}" for i in range(1, NBITS + 1)]
...
def make_descriptor_xml(output_path):
    """Create a custom descriptors.xml enabling only SubstructureFingerprintCount."""
    import importlib
...
def main():
    ...
    print(f"\nRunning PaDEL batch Substructure Count calculation (SMILES input)...")
    start = time.time()
    ...
    for fr in fp_rows:
        ...
        fp_map[name] = fp_dict
    print(f"  Mapped {len(fp_map)} molecules to SubFPC")
    ...
    print(f"  Columns: {len(out_fieldnames)} (1 ID + {NBITS} SubFPC counts)")
    print(f"  Rows with SubFPC: {len(rows) - missing_fp}")
    print(f"  Rows without SubFPC: {missing_fp}")
