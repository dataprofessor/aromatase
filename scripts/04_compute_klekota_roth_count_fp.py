"""
Compute Klekota-Roth Count (4860 counts) using PaDEL batch mode with SMILES input.

Uses PaDEL-Descriptor's KlekotaRothFingerprintCount. These are frequency counts (not binary).
...
OUTPUT_FILE = os.path.join(_DIR, "..", "data", "fingerprints", "aromatase_klekota_roth_count_fp.csv")

NBITS = 4860
FP_COLS = [f"KRFPC{i}" for i in range(1, NBITS + 1)]
...
def make_descriptor_xml(output_path):
    """Create a custom descriptors.xml enabling only KlekotaRothFingerprintCount."""
    import importlib
...
def main():
    ...
    print(f"\nRunning PaDEL batch Klekota-Roth Count calculation (SMILES input)...")
    start = time.time()
    ...
    for fr in fp_rows:
        ...
        fp_map[name] = fp_dict
    print(f"  Mapped {len(fp_map)} molecules to KRFPC")
    ...
    print(f"  Columns: {len(out_fieldnames)} (1 ID + {NBITS} KRFPC counts)")
    print(f"  Rows with KRFPC: {len(rows) - missing_fp}")
    print(f"  Rows without KRFPC: {missing_fp}")
