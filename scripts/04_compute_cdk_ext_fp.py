"""
Compute CDK Extended Fingerprinter (1024 bits) using PaDEL batch mode with SMILES input.

Uses PaDEL-Descriptor's ExtendedFingerprinter. SMILES (.smi) input is used instead of
SDF to ensure consistent aromaticity perception.

Note: A custom descriptors.xml is generated at runtime to enable
ExtendedFingerprinter and disable other fingerprint types.
"""
...
OUTPUT_FILE = os.path.join(_DIR, "..", "data", "fingerprints", "aromatase_cdk_ext_fp.csv")

NBITS = 1024
FP_COLS = [f"ExtFP{i}" for i in range(1, NBITS + 1)]
...
def make_descriptor_xml(output_path):
    """Create a custom descriptors.xml enabling only ExtendedFingerprinter."""
    import importlib
...
def main():
    ...
    # Run PaDEL batch
    print(f"\nRunning PaDEL batch CDK Extended Fingerprinter calculation (SMILES input)...")
    start = time.time()
    ...
    print(f"  Mapped {len(fp_map)} molecules to ExtFP")
    ...
    print(f"  Columns: {len(out_fieldnames)} (1 ID + {NBITS} ExtFP bits)")
    print(f"  Rows with ExtFP: {len(rows) - missing_fp}")
    print(f"  Rows without ExtFP: {missing_fp}")
