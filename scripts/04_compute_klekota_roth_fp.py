"""
Compute Klekota-Roth fingerprints (4860 bits) using PaDEL batch mode with SMILES input.

Uses PaDEL-Descriptor's KlekotaRothFingerprinter (4860 SMARTS substructure
patterns defined by Klekota & Roth). SMILES (.smi) input is used instead of
SDF to ensure consistent aromaticity perception.

Note: A custom descriptors.xml is generated at runtime to enable
KlekotaRothFingerprinter and disable other fingerprint types.
"""
...
OUTPUT_FILE = os.path.join(_DIR, "..", "data", "fingerprints", "aromatase_klekota_roth_fp.csv")

NBITS = 4860
KRFP_COLS = [f"KRFP{i}" for i in range(1, NBITS + 1)]
...
def make_descriptor_xml(output_path):
    """Create a custom descriptors.xml enabling only KlekotaRothFingerprinter."""
    import importlib
    ...
    # Enable KlekotaRothFingerprinter
    xml = xml.replace(
...
def main():
    ...
    # Run PaDEL batch
    print(f"\nRunning PaDEL batch Klekota-Roth FP calculation (SMILES input)...")
    start = time.time()
    ...
    # Build lookup: chembl_id -> KRFP dict
    fp_map = {}
    for fr in fp_rows:
        name = fr.get("Name", "")
        fp_dict = {c: fr.get(c, "0") for c in KRFP_COLS}
        fp_map[name] = fp_dict

    print(f"  Mapped {len(fp_map)} molecules to KRFP")

    # Build output: molecule_chembl_id + KRFP columns only
    out_fieldnames = ["molecule_chembl_id"] + KRFP_COLS
    ...
    for row in rows:
        ...
        if fp:
            ...
            missing_fp += 1
            row.update({c: "" for c in KRFP_COLS})
    ...
    print(f"  Columns: {len(out_fieldnames)} (1 ID + {NBITS} KRFP bits)")
    print(f"  Rows with KRFP: {len(rows) - missing_fp}")
    print(f"  Rows without KRFP: {missing_fp}")

    # Bit density
    on_counts = []
    for row in rows:
        bits_on = sum(1 for c in KRFP_COLS if row.get(c) == "1")
    ...
    bit_sums = {c: 0 for c in KRFP_COLS}
