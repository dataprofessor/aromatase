"""
Compute AtomPairs2D Fingerprinter (780 bits) using PaDEL batch mode with SMILES input.

Uses PaDEL-Descriptor's AtomPairs2DFingerprinter. SMILES (.smi) input is used instead of
SDF to ensure consistent aromaticity perception.

Note: A custom descriptors.xml is generated at runtime to enable
AtomPairs2DFingerprinter and disable other fingerprint types.
"""
...
OUTPUT_FILE = os.path.join(_DIR, "..", "data", "fingerprints", "aromatase_atompairs2d_fp.csv")

NBITS = 780
FP_COLS = [f"AD2D{i}" for i in range(1, NBITS + 1)]
...
def make_descriptor_xml(output_path):
    """Create a custom descriptors.xml enabling only AtomPairs2DFingerprinter."""
    import importlib
    ...
    # Enable AtomPairs2DFingerprinter
    xml = xml.replace(
...
def main():
    ...
    # Run PaDEL batch
    print(f"\nRunning PaDEL batch AtomPairs2D Fingerprinter calculation (SMILES input)...")
    start = time.time()
    ...
    print(f"  Mapped {len(fp_map)} molecules to AD2D")
    ...
    print(f"  Columns: {len(out_fieldnames)} (1 ID + {NBITS} AD2D bits)")
    print(f"  Rows with AD2D: {len(rows) - missing_fp}")
    print(f"  Rows without AD2D: {missing_fp}")
