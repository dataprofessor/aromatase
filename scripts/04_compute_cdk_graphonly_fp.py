"""
Compute CDK GraphOnly Fingerprinter (1024 bits) using PaDEL batch mode with SMILES input.

Uses PaDEL-Descriptor's GraphOnlyFingerprinter. SMILES (.smi) input is used instead of
SDF to ensure consistent aromaticity perception.

Note: A custom descriptors.xml is generated at runtime to enable
GraphOnlyFingerprinter and disable other fingerprint types.
"""
...
OUTPUT_FILE = os.path.join(_DIR, "..", "data", "fingerprints", "aromatase_cdk_graphonly_fp.csv")

NBITS = 1024
FP_COLS = [f"GraphFP{i}" for i in range(1, NBITS + 1)]
...
def make_descriptor_xml(output_path):
    """Create a custom descriptors.xml enabling only GraphOnlyFingerprinter."""
    import importlib
...
def main():
    ...
    # Run PaDEL batch
    print(f"\nRunning PaDEL batch CDK GraphOnly Fingerprinter calculation (SMILES input)...")
    start = time.time()
    ...
    # Build lookup: chembl_id -> FP dict
    fp_map = {}
    for fr in fp_rows:
        name = fr.get("Name", "")
    ...
    print(f"  Mapped {len(fp_map)} molecules to GraphFP")

    # Build output: molecule_chembl_id + FP columns only
    ...
    print(f"  Columns: {len(out_fieldnames)} (1 ID + {NBITS} GraphFP bits)")
    print(f"  Rows with GraphFP: {len(rows) - missing_fp}")
    print(f"  Rows without GraphFP: {missing_fp}")

    # Bit density
    on_counts = []
    for row in rows:
        bits_on = sum(1 for c in FP_COLS if row.get(c) == "1")
    ...
    print(f"\n  Top 10 most common GraphFP bits:")
