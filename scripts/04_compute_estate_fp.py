"""
Compute E-State Fingerprinter (79 bits) using PaDEL batch mode with SMILES input.

Uses PaDEL-Descriptor's EStateFingerprinter. SMILES (.smi) input is used instead of
SDF to ensure consistent aromaticity perception.

Note: A custom descriptors.xml is generated at runtime to enable
EStateFingerprinter and disable other fingerprint types.
"""
...
OUTPUT_FILE = os.path.join(_DIR, "..", "data", "fingerprints", "aromatase_estate_fp.csv")

NBITS = 79
FP_COLS = [f"EStateFP{i}" for i in range(1, NBITS + 1)]
...
def make_descriptor_xml(output_path):
    """Create a custom descriptors.xml enabling only EStateFingerprinter."""
    import importlib
...
def main():
    ...
    # Run PaDEL batch
    print(f"\nRunning PaDEL batch E-State Fingerprinter calculation (SMILES input)...")
    start = time.time()
    ...
    print(f"  Mapped {len(fp_map)} molecules to EStateFP")
    ...
    print(f"  Columns: {len(out_fieldnames)} (1 ID + {NBITS} EStateFP bits)")
    print(f"  Rows with EStateFP: {len(rows) - missing_fp}")
    print(f"  Rows without EStateFP: {missing_fp}")
    ...
    print(f"\n  Top 10 most common EStateFP bits:")
